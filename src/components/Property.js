...
const Property = ({ property, buttonText, buttonAction }) => {
  ...
  <Card fluid className="property" key={property.id}>
    <Card.Header className="property-header" style={headerStyle} >
      <img src={property.agency.logo} className="agency-logo" alt="Agency logo" />
    </Card.Header>
    <Image src={property.mainImage} className="main-image" alt="Main" />
    <Card.Content>
      <p className="left floated price">{property.price}</p>
      <Button primary className="right floated button-action" onClick={() => buttonAction(property.id)}>{buttonText}</Button>
    </Card.Content>
  </Card>
)};
